...
.Ackleys <- "SEXP Ackleys(const std::vector< double >& X){
  const int N = X.size();
...
.AluffiPentini <- "SEXP AluffiPentini(const std::vector< double >& X){
...
.BeckerLago <- 'SEXP BeckerLago(const std::vector< double >& X){
...
.Bohachevsky1 <- 'SEXP Bohachevsky1(const std::vector< double >& X){
  if (X.size() != 2) Rcpp::stop("Bohachevsky 1 needs two variables");
  const double f = pow(X[0],2) + 2.*X[1]*X[1] - .3*cos(3*arma::datum::pi*X[0]) -.4*cos(4*arma::datum::pi*X[1]) + .7;
  return(Rcpp::wrap(f));
}'


.Bohachevsky2 <- 'SEXP Bohachevsky2(const std::vector< double >& X){
  if (X.size() != 2) Rcpp::stop("Bohachevsky 2 needs two variables");
  const double f =  pow(X[0],2) + 2.*X[1]*X[1] - .3*cos(3*arma::datum::pi*X[0])*cos(4*arma::datum::pi*X[1]) + .3;
  return(Rcpp::wrap(f));
}'


.Branin <- 'SEXP Branin(const std::vector< double >& X,
            const double a=1., 
            const double b=5.1/(4*arma::datum::pi*arma::datum::pi), 
            const double c=5/arma::datum::pi,
            const double d=6., 
            const double e = 10., 
            const double f = 1/(8*arma::datum::pi)
){
...
.CosMixN <- 'SEXP CosMixN(const std::vector< double >& X){
  double sum1=0., sum2=0;
  for (auto it = X.begin(); it != X.end(); it++){
    sum1 += cos(5*arma::datum::pi*(*it));
    sum2 += pow((*it),2);
  }
...
.DekkersAarts <- 'SEXP DekkersAarts(const std::vector< double >& X){
...
.Easom <- 'SEXP Easom(const std::vector< double >& X){
  if (X.size() != 2) Rcpp::stop("Easom needs two variables");
  const double f = -cos(X[0])*cos(X[1])*exp(-pow(X[0]-arma::datum::pi,2)-pow(X[1]-arma::datum::pi,2));
  return(Rcpp::wrap(f));
}'

.EMichalewicz <- 'SEXP EMichalewicz(const std::vector< double >& X,
                  const double m = 10.){
  const double cost = cos(arma::datum::pi/6.);
  const double sint = sin(arma::datum::pi/6.);
  const int nn = X.size();
  double y[nn];
  int j = 0;
  ...
  double f = 0;
  for (j = 0; j < nn; j++)
    f -= sin(y[j]) * pow(sin((j+1)*y[j]*y[j]/arma::datum::pi),2.0*m);
  return(Rcpp::wrap(f));
}'


.Expo <- 'SEXP Expo(const std::vector< double >& X){
  double s = 0;
  for (auto it = X.begin(); it != X.end(); it++)
  {
    s += (*it)*(*it);
...
.GoldPrice <- 'SEXP GoldPrice(const std::vector< double >& X){
...
.Griewank <- 'SEXP Griewank(const std::vector< double >& X){
  double s = 0.;
  double p = 1.;
  for (auto it = X.begin(); it != X.end(); ++it)
  { 
...
.Gulf <- 'SEXP Gulf(const std::vector< double >& X){
...
.Hartman3 <- 'SEXP Hartman3(const std::vector< double >& X){
...
.Hartman6 <- 'SEXP Hartman6(const std::vector< double >& X){
...
.Hosaki <- 'SEXP Hosaki(const std::vector< double >& X){
...
.Kowalik <- 'SEXP Kowalik(const std::vector< double >& X){
...
.LM1 <- 'SEXP LM1(const std::vector< double >& X){
  int j, nn;
  double zw1, zw2;
  nn = X.size();
  zw1 = 10*pow(sin(arma::datum::pi*(1+.25*(X[0]+1))),2);
  zw2 = pow(.25*(X[(int)nn-1]+1),2);
  
  for (j=0; j < (nn-1); j++)
  {
    zw1 += pow(.25*(X[j]+1),2)*(1+pow(sin(arma::datum::pi*(.25*X[j+1])),2));
  }
  
  const double f = arma::datum::pi/(double)nn*(zw1+zw2);
  return(Rcpp::wrap(f));
}'

.McCormic <- 'SEXP McCormic(const std::vector< double >& X){
  if (X.size() != 2) Rcpp::stop("McCormic needs 2 variables");
  const double f = sin((X[0]+X[1])) + pow(X[0]-X[1],2) - 1.5*X[0] + 2.5*X[1] + 1.;
  return(Rcpp::wrap(f));
}'

.MeyerRoth <- 'SEXP MeyerRoth(const std::vector< double >& X)
...
.MieleCantrell <- 'SEXP MieleCantrell(const std::vector< double >& X){
...
.Modlangerman <- 'SEXP Modlangerman(const std::vector< double >& X){
  ...
  for (i=0; i<5; i++)
  {
    ...
    f-=c[i]*(exp(-dist/arma::datum::pi)*cos(arma::datum::pi*dist));
  }
  return(Rcpp::wrap(f));
}'

.ModRosenbrock <- 'SEXP ModRosenbrock(const std::vector< double >& X){
...
.MultiGauss <- 'SEXP MultiGauss(const std::vector< double >& X){
...
.Neumaier2 <- 'SEXP Neumaier2(const std::vector< double >& X){
...
.Neumaier3 <- 'SEXP Neumaier3(const std::vector< double >& X){
  int i;
...
.Paviani <- 'SEXP Paviani(const std::vector< double >& X){
  int j;
...
.PriceTransistor <- 'SEXP PriceTransistor(const std::vector< double >& X){
...
.Rastrigin <- 'SEXP Rastrigin(const std::vector< double >& X){
  ...
  for (j = 0; j < X.size(); j++)
  {
    f += X[j]*X[j]-10.*cos(2.*arma::datum::pi*X[j])+10.;
  }
  return(Rcpp::wrap(f));
}'

.Rosenbrock <- 'SEXP Rosenbrock(const std::vector< double >& X){
  int j, nn;
  double a = 0.,b = 0.;
  double f = 0.;
  nn = X.size();
...
.Salomon <- 'SEXP Salomon(const std::vector< double >& X){
  int j;
  double s = 0;
  ...
  const double f = -cos(2.*arma::datum::pi*sqrt(s))+.1*sqrt(s)+1.;
  return(Rcpp::wrap(f));
}'

.Schaffer1 <- 'SEXP Schaffer1(const std::vector< double >& X){
...
.Schaffer2 <- 'SEXP Schaffer2(const std::vector< double >& X){
...
.Schubert <- 'SEXP Schubert(const std::vector< double >& X){
  int i,j;
  double prod = 1.;
  for (i = 0; i < X.size(); i++)
  {
...
.Schwefel <- 'SEXP Schwefel(const std::vector< double >& X){
  int j;
  double s = 0.;
...
.Shekel10 <- 'SEXP Shekel10(const std::vector< double >& X){
...
.Shekel5 <- 'SEXP Shekel5(const std::vector< double >& X){
...
.Shekel7 <- 'SEXP Shekel7(const std::vector< double >& X){
...
.Shekelfox5 <- 'SEXP Shekelfox5(const std::vector< double >& X){
...
.Wood <- 'SEXP Wood(const std::vector< double >& X){
...
.Zeldasine <- 'SEXP Zeldasine(const std::vector< double >& X,
               const double A = 2.5, 
               const double B = 5.
){
  int j;
  double prod1=1., prod2=1.;
  const double z = arma::datum::pi/6;
